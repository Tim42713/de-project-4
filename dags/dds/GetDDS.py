...
class StgDataLoader:
    def __init__(self, stg: PgConnect) -> None:
        ...
        self.wf_key_couriers = "couriers_dds"
        self.wf_key_deliveries = "deliveries_dds"
        self.wf_key_restaurants = "restaurants_dds_"
        self.batch = 1
    ...
    def get_restaurants(
        ...
        with conn.cursor(row_factory=class_row(Restaurants)) as cur:
            cur.execute(
                """
                   SELECT id, object_value::json->>'_id' AS restaurant_id, object_value::json->>'name' AS restaurant_name 
                   FROM stg.restaurants WHERE id > %(threshold)s ORDER BY id ASC LIMIT %(limit)s;
                """,
                {"threshold": threshold, "limit": limit},
            )
            obj = cur.fetchall()
    ...
    def insert_restaurants(self, conn: PgConnect, restaurant: Restaurants) -> None:
        with conn.cursor() as cur:
            cur.execute(
                """
    ...
    def load_restaurants(self):
        with self.stg.connection() as conn:
            ...
            last_loaded_offset = wf_setting.workflow_settings["last_loaded_offset"]
            load_queue = self.get_restaurants(
                conn, threshold=last_loaded_offset, limit=self.batch
            )
            for load_data in load_queue:
                    restaurants = Restaurants(
                        id=load_data.id,
                        restaurant_id=load_data.restaurant_id,
                        restaurant_name=load_data.restaurant_name,
                    )
                    self.insert_restaurants(conn, restaurants)  
        ...
        with conn.cursor(row_factory=class_row(Delivery)) as cur:
            cur.execute(
                """
                   ...
                   INNER JOIN dds.dm_orders do2 ON cte.order_id = do2.order_id 
