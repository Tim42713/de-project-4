...
class RawRestaurants(BaseModel):
    restaurant_id: str
    object_value: str
...
class LoadAPI:
    def __init__(self, stg: PgConnect) -> None:
        ...
        self.wf_key_restaurants = "rest_stg"
        self.wf_key_couriers = "courier_stg"
        self.wf_key_deliveries = "deliver_stg"
        self.api_nickname = "timur_bynin"
        self.api_cohort = "15"
    ...
    def get_rest(self, offset: int) -> List[Dict]:
        response = get(
            "https://d5d04q7d963eapoepsqr.apigw.yandexcloud.net/restaurants?sort_field=id&sort_direction=asc&limit=50"
    ...
    def insert_rest(self, conn, raw_restaurant: RawRestaurants) -> None:
        with conn.cursor() as cur:
            cur.execute(
                """
                    INSERT INTO stg.restaurants(restaurant_id, object_value)
                    VALUES (%(restaurant_id)s, %(object_value)s)
                    ON CONFLICT (restaurant_id) DO UPDATE
                    SET
                        object_value = EXCLUDED.object_value
                """,
                {
                    "restaurant_id": raw_restaurant.restaurant_id,
                    "object_value": raw_restaurant.object_value,
                },
            )
    ...
    def insert_deliveries(self, conn, raw_delivery: RawDelivery) -> None:
        with conn.cursor() as cur:
            cur.execute(
                """
    ...
    def load_rest(self):
        with self.stg.connection() as conn:
            ...
            while True:
                load_queue = self.get_rest(last_loaded_offset)
                if len(load_queue) == 0:
                    break
                else:
                    for load_data in load_queue:
                        raw_restaurants = RawRestaurants(
                            restaurant_id=load_data["_id"],
                            object_value=json.dumps(load_data),
                        )
                        self.insert_rest(conn, raw_restaurants)
